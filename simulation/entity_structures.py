...
class Animal(Entity):
    ...
    def __post_init__(self):
        ...
        self.target = self
        self.task = Task.wander #Default task at start
        self.age = 0 #Default age at start
        self.chased = False #Default value for chased
        self.hunt_per_day = 0
    ...
    def update_task(self,delta_time):
        ...
        if self.task_predictor is None:
            ...
            self.train_task_predictor(self.task_predictor, criterion, optimizer, num_epochs=200)
    ...
    def update(self,delta_time):
        ...
        match self.task:
            ...
            case Task.reproduce:
                ...
                targets = list()
                for entity in self.entity_manager.entities:
                    if isinstance(entity, Animal) and not id(entity) is id(self) and entity.animal_type == self.animal_type:   
                        targets.append(entity)
                if not targets:
                    self.update_task(delta_time)
                    return
